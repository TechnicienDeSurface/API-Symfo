...
class VideoGameRepository extends ServiceEntityRepository
{
    ...
    public function findReleaseGamesNextSevenDays(): array
    {
        $today = new \DateTime('today');
        $sevenDaysLater = (new \DateTime('today'))->modify('+7 days');

        return $this->createQueryBuilder('v')
            ->where('v.releaseDate >= :today')
            ->andWhere('v.releaseDate <= :sevenDays')
